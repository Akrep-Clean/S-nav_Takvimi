...
class SeatingPlanWindow:
    def __init__(self, parent, exam_details, department_id):
        ...
        self.seating_plan_by_classroom = defaultdict(list)
        ...
        self.canvas = None
        self.classroom_select_combo = None
    ...
    def generate_seating_plan(self):
        try:
            ...
            exam_student_count = int(self.exam_details.get('student_count', 0))
            ...
            for code in self.classroom_codes_list:
                ...
                if classroom_info:
                    self.classroom_info_map[code] = {
                        "rows": int(classroom_info[0]),
                        "cols": int(classroom_info[1]),
                        "capacity": int(classroom_info[2]),
                        "seat_type": classroom_info[3]
                    }
                    total_available_capacity += int(classroom_info[2])
                else:
            ...
            for code in self.classroom_codes_list:
                ...
                room_info = self.classroom_info_map[code]
                rows, cols, capacity = room_info["rows"], room_info["cols"], room_info["capacity"]
            ...
            if total_placed_students < db_student_count:
                 messagebox.showwarning("Kapasite Aşıldı", 
                                       f"Toplam derslik kapasitesi ({total_available_capacity}) yetersiz!\n\n"
                                       f"{db_student_count} öğrenciden {total_placed_students} tanesi yerleştirilebildi.\n"
                                       f"{db_student_count - total_placed_students} öğrenci açıkta kaldı.")
            else:
    ...
    def update_canvas_preview(self, event):
        ...
        room_info = self.classroom_info_map[selected_code]
        rows, cols, capacity, seat_type = room_info["rows"], room_info["cols"], room_info["capacity"], room_info["seat_type"]
    ...
    def draw_seating_plan_on_canvas(self, rows, cols, seat_type, students_assigned, capacity):
        ...
        seat_width = total_cell_width * 0.8
        seat_height = total_cell_height * 0.8
        h_gap = total_cell_width * 0.1
        v_gap = total_cell_height * 0.1
        
        font_size = min(9, int(seat_height / 3.5))
        seat_font = ("Arial", font_size) # "Çince" hatası için Arial
        ...
        for r in range(1, rows + 1):
            for c in range(1, cols + 1):
                ...
                fill_color = "lightblue" if student_no else "#E0E0E0"
                self.canvas.create_rectangle(x1, y1, x2, y2, fill=fill_color, outline="black")

                if student_no:
                    cx = (x1 + x2) / 2
                    cy = (y1 + y2) / 2
                    short_no = str(student_no)[-4:]
                    try:
                        self.canvas.create_text(cx, cy, text=short_no, font=seat_font, anchor="center")
                    except Exception as e:
                        print(f"Canvas text çizim hatası (font: {seat_font}): {e}")
